...
import { chatReadOnlyReason, isWritableClaudeChat } from './claude-chat'
...
function modelLabel(model: string) { return modelNames[model] ?? model }
...
export default function App() {
  ...
  function openNameSuggestions(candidates: SessionNameCandidate[]) {
    const supported = candidates.filter(candidate => candidate.target.kind === 'native'
      ...
      : isWritableClaudeChat(state.chats.find(item => item.id === candidate.target.id) ?? { model: '' }))
  ...
  async function saveRename(event: FormEvent) {
    event.preventDefault(); if (preview || busy || !titleInput.trim() || (renameTarget.kind === 'chat' && !isWritableClaudeChat(state.chats.find(item => item.id === renameTarget.id) ?? { model: '' })) || (renameTarget.kind === 'native' && Boolean(nativeSessions.find(item => item.sessionId === renameTarget.id)?.readOnlyReason))) return
    const requestId = navigation.current
    setBusy(true); setError('')
    try {
  ...
  return <div className={`workspace ${sidebarHidden ? 'sidebar-hidden' : ''} ${preview ? 'preview-mode' : ''}`}>
    ...
    <aside className={`sidebar ${sidebarOpen ? 'sidebar-open' : ''}`} aria-label="Workspace navigation">
      <div className="brand-row"><button className="brand" onClick={() => newChat()}><ClaudeMark /><span>ARRA Claude Code</span></button><IconButton icon="search" label="Search projects and Oracles (⌘K)" onClick={openProjectSearch} /><span className="mobile-only"><IconButton icon="close" label="Close sidebar" onClick={() => setSidebarOpen(false)} /></span></div>
      ...
      <nav className="sidebar-scroll"><section className="projects"><div className="section-label"><span>Projects</span><span className="repository-actions">{!preview && <IconButton icon="refresh" label="Refresh repositories and threads" onClick={() => { void refreshRepositories(); void refreshNative() }} disabled={repositoriesLoading || nativeListLoading} />}<IconButton icon="plus" label="Add project" onClick={() => { setProjectName(''); setProjectPath(health?.cwd || ''); setError(''); setModal('project') }} disabled={preview} /></span></div>
        ...
        <section className="recents"><div className="section-label"><span>Picked up here</span></div>{state.chats.map(item => <SidebarThread key={item.id} title={item.title} selected={chat?.id === item.id && !preview} running={item.status === 'running'} existingTerminal={isWritableClaudeChat(item) ? existingTerminalFor(item.sessionId) : undefined} onCopyExistingTerminal={item.sessionId && isWritableClaudeChat(item) ? () => void copyExistingTerminal(item.sessionId!) : undefined} onSelect={() => selectChat(item)} onRename={preview || !isWritableClaudeChat(item) ? undefined : () => openRename({ kind: 'chat', id: item.id }, item.title)} renameDisabled={busy || item.status === 'running'} />)}{loaded && !state.chats.length && <p className="sidebar-empty">A fresh start. Your next chat goes here.<button onClick={showAgents}>Find your other chats <Icon name="chevron" size={12} /></button></p>}</section>
      </nav>

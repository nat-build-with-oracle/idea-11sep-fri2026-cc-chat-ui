...
export default function App() {
  ...
  async function loadMore() {
    ...
    if (!id || nextHistory === null || busy || running) return
    const requestId = navigation.current
    const requestView = view
    const offset = nextHistory
    setBusy(true)
    try {
      if (requestView === 'native') {
        const page = await api.nativeHistory(id, offset)
        if (requestId !== navigation.current) return
        nearBottom.current = false
        setNativeMessages(previous => [...previous, ...page.messages]); setNativeOffset(page.nextOffset)
      } else if (chat) {
        const updated = await api.loadChatHistory(chat.id)
        if (requestId === navigation.current) nearBottom.current = false
        applyChat(updated)
      }
    } catch (reason) { if (requestId === navigation.current) setError(errorMessage(reason)) } finally { setBusy(false) }
  }
  async function send(event: FormEvent) {
    ...
    if (content === '/rename') { setTitleInput(chat?.title || newTitle); setModal('rename'); return }
  ...
  function openRename() { setTitleInput(currentTitle === 'New conversation' ? '' : currentTitle); setError(''); setModal('rename') }
  async function saveRename(event: FormEvent) {
    event.preventDefault(); if (preview || busy || !titleInput.trim()) return
    const requestId = navigation.current
    setBusy(true); setError('')
    try {
      if (view === 'native' && native?.sessionId) {
        const result = await api.renameNative(native.sessionId, titleInput.trim())
        if (requestId === navigation.current) setNative(result.session)
      ...
      else if (chat) applyChat(await api.updateChat(chat.id, { title: titleInput.trim() }))
            ...
            <div className="conversation-scroll" ref={scroller} onScroll={event => { const node = event.currentTarget; nearBottom.current = node.scrollHeight - node.scrollTop - node.clientHeight < 100 }}>
              {selectedId && !chat ? <div className="list-empty"><h2>{loaded ? 'Conversation not found' : 'Loading conversation…'}</h2>{loaded && <><p>It may have been removed from this workspace.</p><button className="subtle-button" onClick={showAgents}>Go to Your chats</button></>}</div> : messages.length ? <div className="conversation" aria-label="Conversation">{conversationItems.map(item => {
                ...
                return <article className={`message ${message.role === 'user' && !toolOnly ? 'user-message' : 'assistant-message'} ${toolOnly ? 'tool-only-message' : ''}`} key={item.key}>
                  ...
                  {!toolOnly && <div className="message-meta">{!message.history && <time dateTime={message.createdAt}>{timeLabel(message.createdAt)}</time>}{message.content && <IconButton icon="copy" label="Copy message" onClick={() => void copy(message.content, 'Message copied')} />}</div>}
                </article>
              })}{latestAssistant && !latestAssistant.usage && <p className="usage-unavailable">{latestAssistant.status === 'streaming' ? 'Token usage will appear when this turn finishes.' : 'Token usage was not reported for this response.'}</p>}{nextHistory !== null && <button className="load-history subtle-button" onClick={() => void loadMore()} disabled={busy || running}>{busy ? 'Loading…' : 'Load more conversation history'}<Icon name="chevron" size={14} /></button>}</div> : nativeLoading ? <div className="conversation"><div className="skeleton-lines"><i /><i /><i /></div></div> : <div className="empty-state"><ClaudeMark size={46} /><h2>{view === 'native' ? 'No messages to display' : 'What’s the move?'}</h2><p>{view === 'native' ? 'This session has no readable conversation messages yet.' : 'A wild idea, a tiny fix, or the next big thing.'}</p></div>}
            </div>

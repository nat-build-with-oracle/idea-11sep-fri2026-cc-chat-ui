...
[← README](../README.md) · [Claude Code and AI naming](claude-code-and-session-names.md) · [Tutorial](tutorials/using-claude-code-chat-ui.md) · [Deployment and troubleshooting](cloudflare.md)
...
- **Live Timeline:** the sidebar opens Timeline in the same tab on the selected backend's machine and preserves the full current URL in `returnTo`. Browser **Back** returns to the exact ARRA session. Local access uses port 47881; configured VPN access uses the app-managed bridge on port 47882 with the same authentication mode and browser session as ARRA. The existing Timeline service must already be running on the backend machine's localhost:47881. Its source and session-sync process are unchanged. Timeline's own return-link support is separate; use browser Back for remote return navigation.
...
- **Suggest session names:** explicitly generate three choices for one selected thread using a bounded, two-call, no-tools naming pipeline, then save only the ARRA display alias. No automatic rename or native transcript mutation occurs. See [AI-assisted session naming](claude-code-and-session-names.md#suggest-session-names-with-ai).
